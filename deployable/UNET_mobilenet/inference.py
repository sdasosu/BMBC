...
class_labels = [class_labels_dict[key] for key in sorted(class_labels_dict, key=lambda x: int(x))]
#------------------------------------------------------------------------

#-------------------- Model ---------------------------
# Build the UNet model using MobileNetV3 small encoder
model = smp.Unet(
...
# For inference acceleration, we trace the model with a sample input.
...
def process_image(image_path):
    ...
    input_batch = input_tensor.unsqueeze(0).to(device)  # Create a mini-batch for the model
    ...
    with torch.no_grad():
        ...
        # For segmentation, obtain the pixel-wise prediction then determine the dominant class
        predicted_classes = torch.argmax(probabilities, dim=1)
        predicted_class = predicted_classes.flatten().mode()[0].item()

        # Map predicted index to label (if within range)
        ...
        if confidence < 0.75:  # Confidence threshold (adjust as needed)
        ...
        if predicted_label in ["Others", "adult", "egg masses", "instar nymph (1-3)", "instar nymph (4)"]:
            save_path = 'results/detected_images'
            os.makedirs(save_path, exist_ok=True)
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            image.save(os.path.join(save_path, f'{predicted_label}_{confidence:.4f}_{timestamp}.jpg'))
