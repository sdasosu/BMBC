        ...
        for batch_idx, (images, masks) in enumerate(test_loader):
            ...
            for t in range(num_classes):
                for p in range(num_classes):
                    confusion_matrix[t, p] += torch.sum((mask_flat == t) & (pred_flat == p))
            ...
            for i in range(min(images.size(0), max_vis_images - vis_count)):
                ...
                plt.savefig(os.path.join(save_dir, f'result_img{vis_count}.png'),
...
def main():
    ...
    checkpoint_files = sorted(glob.glob('models/model_epoch_*.pth'), 
    ...
    for checkpoint_file in checkpoint_files:
        ...
        try:
            ...
            for i, class_name in enumerate(class_names):
                print(f'  {class_name:15s} - IoU: {mean_iou_per_class[i]:.4f}, Acc: {per_class_acc[i]:.4f}')
    ...
    for class_name in class_names:
        print(f'  {class_name:15s} - IoU: {best_epoch_data[f"iou_{class_name}"]:.4f}, '
